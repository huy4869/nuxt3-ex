<template>
  ...
  <el-form-item :prop="nameRef" :error="errorResponse" :rules="rules">
    <el-select
      ref='refSelect'
      v-model="value"
      :class="[isFullWidth ? 'w-full h-full' : '', classExtra]"
    ...
    >
      ...
      <div class="flex items-center gap-2">
        <BaseAvatar  v-if="option.id !== ''" :uri="option.avatar_thumb" />
        <span class="text-select-name text-body-01 text-color-dark">{{ option.name }}</span>
      </div>
      </el-option>
    </el-select>
  </el-form-item>
...
const emits = defineEmits(['update:model', 'handleRemoteMethod', 'changeSelect'])
...
const handleValidateSelect = (event, key, ruleForm) => {
  if(Object.entries(ruleForm).length !== 0) {
    if (!event) {
